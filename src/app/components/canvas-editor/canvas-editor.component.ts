...
export interface CanvasActions {
  getLayers: () => LayerInfo[];
  addRectangle: (opts: RectOptions) => void;
  addPolygon: (opts: PolyOptions) => void;
  updateLayer: (index: number, props: Partial<RectOptions & { visible: boolean }>) => boolean;
  deleteLayer: (index: number) => boolean;
  clearAllLayers: () => void;
  setLayerVisibility: (index: number, visible: boolean) => boolean;
}
...
export class CanvasEditorComponent implements AfterViewInit, OnDestroy {
  ...
  // ─── Agent-callable public methods ───────────────────────────────────────

  /** Returns a serializable snapshot of all canvas layers for the agent. */
  getLayersInfo(): LayerInfo[] {
    if (!this.canvas) return [];
    ...
    return objects.map((obj, index) => {
      ...
      if (obj.type === 'rect') {
        info.width = Math.round((obj as Rect).width || 0);
        info.height = Math.round((obj as Rect).height || 0);
      } else if (obj.type === 'polygon') {
        const poly = obj as Polygon;
        info.points = (poly.points || []).map((p: any) => ({ x: Math.round(p.x), y: Math.round(p.y) }));
      }
      return info;
    });
  }

  /** Programmatically adds a rectangle to the canvas. Called by the agent. */
  addRectangleFromAgent(opts: RectOptions): void {
    if (!this.canvas) return;
    const stroke = opts.stroke || '#4f46e5';
    const fillHex = opts.fill || '#818cf8';
    const opacity = opts.fillOpacity !== undefined ? opts.fillOpacity : 0.2;
    const fill = this.hexToRgba(fillHex, opacity);
    const rect = new Rect({
      left: opts.x,
      top: opts.y,
      width: opts.width,
  ...
  /** Programmatically adds a polygon to the canvas. Called by the agent. */
  addPolygonFromAgent(opts: PolyOptions): void {
    if (!this.canvas || opts.points.length < 3) return;
    const stroke = opts.stroke || '#4f46e5';
    const fillHex = opts.fill || '#818cf8';
    const opacity = opts.fillOpacity !== undefined ? opts.fillOpacity : 0.2;
    const fill = this.hexToRgba(fillHex, opacity);
    const polygon = new Polygon(opts.points, {
      stroke,
      strokeWidth: opts.strokeWidth || 3,
      fill,
      selectable: true,
  ...
  /** Updates an existing layer by index. Returns false if index is invalid. */
  updateLayerFromAgent(index: number, props: Partial<RectOptions & { visible: boolean }>): boolean {
    const shapes = this.getAgentShapes();
    if (index < 0 || index >= shapes.length) return false;
    const obj = shapes[index];
    const updates: any = {};
    if (props.x !== undefined) updates.left = props.x;
    if (props.y !== undefined) updates.top = props.y;
    if (props.width !== undefined && obj.type === 'rect') updates.width = props.width;
    if (props.height !== undefined && obj.type === 'rect') updates.height = props.height;
    ...
    if (props.fill !== undefined) {
      const opacity = props.fillOpacity !== undefined ? props.fillOpacity : 0.2;
      updates.fill = this.hexToRgba(props.fill, opacity);
    }
    if (props.visible !== undefined) updates.visible = props.visible;
    obj.set(updates);
    obj.setCoords();
    this.canvas?.renderAll();
    this.updateLayersList();
  ...
  /** Deletes a layer by index. Returns false if index is invalid. */
  deleteLayerFromAgent(index: number): boolean {
    const shapes = this.getAgentShapes();
    if (index < 0 || index >= shapes.length) return false;
    const obj = shapes[index];
    this.canvas?.remove(obj);
  ...
  /** Removes all shapes from the canvas (keeps background image). */
  clearAllLayersFromAgent(): void {
    const shapes = this.getAgentShapes();
    shapes.forEach((obj) => this.canvas?.remove(obj));
    this.canvas?.discardActiveObject();
    this.canvas?.renderAll();
    this.updateLayersList();
    this.autoSave();
  ...
  /** Shows or hides a layer by index. Returns false if index is invalid. */
  setLayerVisibilityFromAgent(index: number, visible: boolean): boolean {
    const shapes = this.getAgentShapes();
    if (index < 0 || index >= shapes.length) return false;
    const obj = shapes[index];
    obj.set({ visible });
  ...
  /** Returns a CanvasActions object to pass to the copilot service. */
  getCanvasActions(): CanvasActions {
    return {
      getLayers: () => this.getLayersInfo(),
      addRectangle: (opts) => this.addRectangleFromAgent(opts),
      addPolygon: (opts) => this.addPolygonFromAgent(opts),
      updateLayer: (index, props) => this.updateLayerFromAgent(index, props),
      deleteLayer: (index) => this.deleteLayerFromAgent(index),
      clearAllLayers: () => this.clearAllLayersFromAgent(),
      setLayerVisibility: (index, visible) => this.setLayerVisibilityFromAgent(index, visible),
    };
  }

  /** Internal: returns only user-drawn shape objects (no temp drawing aids, no bg image). */
  private getAgentShapes(): any[] {
    if (!this.canvas) return [];
